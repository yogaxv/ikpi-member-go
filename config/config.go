...
import (
	...
	argonDriver "github.com/thomasvvugt/fiber-hashing/driver/argon2id"
	bcryptDriver "github.com/thomasvvugt/fiber-hashing/driver/bcrypt"
...
func New() *Config {
	...
	if err := config.ReadInConfig(); err != nil {
		...
		if !errors.As(err, &notFoundErr) { // Gunakan `errors.As()` untuk menangani wrapped errors
...
func (config *Config) GetHasherConfig() hashing.Config {
	if strings.ToLower(config.GetString("HASHER_DRIVER")) == "bcrypt" {
		return hashing.Config{
			Driver: bcryptDriver.New(bcryptDriver.Config{
				Complexity: config.GetInt("HASHER_ROUNDS"),
			})}
	} else {
		return hashing.Config{
			Driver: argonDriver.New(argonDriver.Config{
