...
```bash
docker build -t ikpi-member-go .
docker run -it --rm --name ikpi-member-go -e DB_HOST=host.docker.internal -e DB_USER=fiber -e DB_PASSWORD=secret -e DB_DATABASE=boilerplate -p 8080:8080 ikpi-member-go
```
